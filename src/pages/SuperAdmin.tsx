...
export const SuperAdmin = () => {
  ...
  const handleExportData = () => {
    const csvContent =
      ...
      users
        .map(
          (u) =>
            `${u.full_name},${u.email},${u.team || "Unassigned"},${u.ranking || 0},${u.activities_count},${new Date(u.created_at).toLocaleDateString("en-US")}`,
        )
        .join("\n");
    ...
    link.setAttribute(
      "download",
      `fler_admin_export_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const unassignedCount =
    users.length - sponsoringCount - logisticsCount - bothCount;
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-black text-text tracking-tight uppercase">
              Control Center
            </h1>
          </div>
          <p className="text-gray-500 font-medium ml-1">
            Universal infrastructure monitoring and system analytics.
          </p>
        </div>
        ...
        <Card className="xl:col-span-8 overflow-hidden border-none shadow-2xl bg-white/70 backdrop-blur-xl ring-1 ring-black/5">
          <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-black text-text uppercase">
                  Leaderboard
                </h2>
                      ...
                      >
                            ...
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-sm ${
                                index === 0
                                  ...
                                  : index === 1
                                    ? "bg-gray-300 text-white shadow-lg shadow-gray-200"
                                    : index === 2
                        ...
                        <td className="px-8 py-5 text-center">
                          <div className="flex flex-col items-center">
                            ...
                            <span className="text-[8px] text-primary/60 font-black uppercase tracking-widest">
                              Points
                            </span>
                          </div>
                        </td>
      ...
      >
        <div className="space-y-4 max-h-[60vh] overflow-y-auto px-4 py-2 custom-scrollbar">
          {allActivities.length > 0 ? (
            allActivities.map((a) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-black text-text uppercase italic">
                      {a.user_name}
                    </p>
            ...
            <p className="text-center text-gray-400 py-12 uppercase font-black text-xs tracking-widest">
              Zero events logged
            </p>
          )}
        </div>
        ...
        <div className="p-6 space-y-6 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-2 font-medium">
              System access is currently governed by email-whitelist Teams
              defined in{" "}
          ...
          <p className="text-[10px] text-gray-400 italic">
            Global permissions are strictly inherited from the identity layer.
          </p>
        </div>
      </Modal>
    </div>
...
const StatCard = ({ icon, label, value, trend, color }: any) => {
  const colorClasses: Record<string, string> = {
    ...
    emerald:
      "bg-emerald-500/5 bg-emerald-50 text-emerald-600 ring-emerald-500/10",
  ...
  return (
    <Card className="p-7 border-none shadow-xl hover:shadow-2xl transition-all group overflow-hidden relative ring-1 ring-black/5 hover:-translate-y-1">
      <div
        className={`absolute top-0 right-0 w-32 h-32 rounded-full -mr-12 -mt-12 transition-transform group-hover:scale-150 duration-1000 ${colorClasses[color].split(" ")[0]}`}
      ></div>
      <div className="flex items-center gap-5 relative z-10 font-black">
        <div
          className={`p-4 rounded-2xl transition-all duration-500 group-hover:rotate-12 group-hover:scale-110 shadow-lg ${colorClasses[color].split(" ").slice(1, 4).join(" ")}`}
        >
        ...
        <div>
          ...
          <h3 className="text-3xl font-black text-text mt-0.5 tracking-tight">
            {value}
          </h3>
        </div>
      </div>
      <div className="mt-6 flex items-center justify-between relative z-10 px-1">
        <span className="text-[11px] font-black text-gray-400 bg-gray-50 px-3 py-1 rounded-full uppercase tracking-tighter shadow-sm">
          {trend}
        </span>
