...
export const Home = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const completionRate = contactCount
        ? Math.round(((signedCount || 0) / contactCount) * 100)
        : 0;
    ...
    <div className="space-y-6 md:space-y-8 w-full max-w-7xl mx-auto px-4 md:px-0 pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-2">
            ...
            <p className="text-white/80 max-w-xl text-sm md:text-base leading-relaxed">
              Supercharge your event pipelines. Track sponsoring, manage
              logistics resources, and coordinate team actions in one central
              platform.
            </p>
          </div>
          <div className="flex flex-wrap gap-3 shrink-0">
            <Button
              variant="outline"
              className="bg-white/10 text-white hover:bg-white/20 border-white/20 text-xs md:text-sm font-semibold rounded-xl h-11"
              onClick={setIsCommandPaletteOpen}
            >
              Search Command Palette (⌘K)
            </Button>
            <Button
              className="bg-white text-primary hover:bg-white/90 text-xs md:text-sm font-semibold rounded-xl h-11 shadow-md shadow-black/10"
        ...
        <Card className="p-5 md:p-6 relative overflow-hidden group border-0 shadow-lg hover:shadow-xl transition-all duration-300 rounded-2xl bg-white">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-1">
              ...
              <h3 className="text-3xl md:text-4xl font-black mt-2 text-text tracking-tighter">
                {stats.contactCount}
              </h3>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <section className="lg:col-span-2 space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-bold flex items-center gap-2 text-text">
              <Calendar className="text-primary" size={20} /> Active Event
              Workspaces
            </h2>
            <Button
              variant="ghost"
              ...
              >
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center justify-between gap-2">
                    <h4 className="font-bold text-text truncate text-base">
                      {event.name}
                    </h4>
                    <Badge
                      variant={
                        event.status === "ongoing" ? "default" : "success"
                      }
                      className="scale-85"
                    >
                      {event.status}
        ...
        <section className="space-y-6">
          <h2 className="text-xl font-bold flex items-center gap-2 text-text">
            <Activity className="text-secondary" size={20} /> Recent Event
            Stream
          </h2>
