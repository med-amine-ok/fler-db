...
Cet événement de deux jours encourage la créativité, la compétition et le travail en équipe en mettant les participants au défi de concevoir des robots autonomes capables de résoudre un labyrinthe dans un temps Bharti, mobilisant ainsi leurs compétences en mécanique, électronique et automatisation.
...
Notre club organise plusieurs événements scientifiques, culturels et même linguistiques tout au long de l’année afin de développer les soft skills des étudiants et former des ingénieurs aux compétences exceptionnelles.

L’un de nos événements majeurs est l’Algerian Engineering Competition (AEC).
AEC est une compétition nationale d’ingénierie organisée simultanément dans plusieurs wilayas du pays. Elle réunit étudiants, jeunes diplômés, chercheurs, start-ups et passionnés de technologie autour de problématiques réelles proposées par des entreprises et industries. Sous la supervision d’experts, les participants développent des solutions technologiques innovantes à travers des ateliers et des retours spécialisés. Les finalistes présentent leurs projets devant un jury qui les évalue selon des critères précis.
...
L’un de nos événements majeurs est Polymaze.
Polymaze est une compétition de robotique de référence qui encourage la créativité et le travail d'équipe. Les participants relèvent le défi de concevoir des robots autonomes, mobilisant des compétences en mécanique, électronique et automatisation dans une ambiance compétitive et innovante.
...
Dans l’attente de votre retour, veuillez agréer, Madame, Monsieur, l’expression de nos salutations distinguées.`;
  ...
  const getEmailTemplate = (evt: Event) => {
    if (EMAIL_TEMPLATES[evt.name]) {
      return EMAIL_TEMPLATES[evt.name];
    }
...
L’un de nos événements majeurs est ${evt.name}.
Cet événement vise à promouvoir l'excellence et l'innovation tout en renforçant les liens entre les étudiants et le monde professionnel.
...
Cordialement,`;
  ...
  const handleCopyEmail = () => {
    if (!event) return;
    const text = getEmailTemplate(event);
    navigator.clipboard.writeText(text);
    setCopiedEmail(true);
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-6 space-y-4">
            ...
            <p className="text-xs text-gray-400">Use this for the initial email contact with companies.</p>
